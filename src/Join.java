...
public class Join {
    public static class JoinCMapper
    ...
    public static class JoinReducer
        ...
        public void reduce(IntWritable key, Iterable<Text> values,
            ...
            for (Text val:values) {
                ...
                else if (strs[0].equals("T")){
                    count++;
                    sum += Double.parseDouble(strs[1]);
                    int tmp = Integer.parseInt(strs[2]);
                    if (min > tmp) min =tmp;
                }
                else info = "!"+val.toString()+"-len-"+strs.length+"!";
            }
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length != 3) {
            System.err.println("Usage: Join <HDFS Customers> <HDFS Transactions> <HDFS output file>");
            System.exit(2);
        }
        Job job = new Job(conf, "Join two tables");
